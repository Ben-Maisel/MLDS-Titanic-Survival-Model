...
---
silent <- function(expr) invisible(capture.output(expr))
...
library(VIM)         # for KNN imputation (knnImputation)

cat("Loaded libraries\n")
```

```{r console-echo, include=FALSE}
# Forward only user-generated print/cat output to the terminal
orig_out <- knitr::knit_hooks$get("output")
knitr::knit_hooks$set(output = function(x, options) {
  # Only forward to terminal if it's user output (cat/print), not code
  if (!options$echo) cat(x)
  orig_out(x, options)
})

knitr::opts_chunk$set(
  echo = FALSE,     # don't show code unless explicitly enabled
  message = FALSE,  # suppress library load chatter in terminal + HTML
  warning = FALSE   # suppress warnings unless you explicitly show them
)
```
...
invisible(capture.output({
  ...
  # KNN imputation
  train_imputed <- VIM::kNN(as.data.frame(train_scaled), k = 5, imp_var = FALSE)
  test_imputed  <- VIM::kNN(as.data.frame(test_scaled),  k = 5, imp_var = FALSE)
